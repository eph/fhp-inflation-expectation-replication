...
xgrid = np.linspace(-2, 4)
sec508 = {}

with saved_figure(f'output/FHP[k={k}]-CG-full.pdf',
                  nrows=2, ncols=2, 
                  sharex=True, sharey=True) as (fig, ax):

    ppc.cg_beta.plot(kind='kde', ax=ax[0,0])
    ax[0,0].axvline(cg_beta, color='black', linewidth=2)
    ax[0,0].set_title(r'$\hat\beta_{CG}^4$: all shocks',size=17)
    mean, std = ppc.cg_beta.mean(), ppc.cg_beta.std()
    ax[0,0].text(-1, 1.5, f'mean = {mean:5.2f}\nstd = {std:5.2f}')
    total = ppc.copy()

    sec508['all shocks'] = gaussian_kde(ppc.cg_beta)(xgrid)
    ...
    for axi, shock, name in zip(ax.reshape(-1)[1:], shocks, names):
        ...
        ppc.cg_beta.plot(kind='kde', ax=axi,linewidth=2)
        total.cg_beta.plot(kind='kde', ax=axi,linewidth=2, color='grey', alpha=0.5)
        axi.axvline(cg_beta, color='black')
        axi.set_title(r'$\hat\beta_{CG}^4$: ' + name + ' only',size=16)
     
        mean, std = ppc.cg_beta.mean(), ppc.cg_beta.std()
        axi.text(-1, 1.5, f'mean = {mean:5.2f}\nstd = {std:5.2f}')

        sec508[f'{name} only'] = gaussian_kde(ppc.cg_beta)(xgrid)
