!!  f2py --fcompiler=intelem -c meyer_gohde_interface.f90 -lmkl_rt -m meyer_gohde_interface
...
module mg
  ...
  double precision, parameter :: verysmall = 0.0!00001d0
  double precision, parameter :: convergence_criteria = 1e-10
  integer :: nunstab = 0
  integer :: zxz = 0 
  integer :: fixdiv = 1
...
contains
  ...
  subroutine find_max_it_fortran(j_matrices,para,npara,neq,neps,max_it)

    integer, intent(in) :: neq, neps, npara
    ...
    do j = 1,5000
       ...
       call j_matrices(para, j, A, B, C, F, G)
       ...
       if (all_max < convergence_criteria) then
          max_it = j
          return
       end if
  ...
  end subroutine find_max_it_fortran
  ...
  subroutine solve_ma_alt_fortran(iiA,iiB,iiC,iiF,iiG,iiN,j_matrices,iiAinf,iiBinf,iiCinf,iiFinf,iiGinf,para,&
       max_it, npara, neq,neps,MA_VECTOR, ALPHA, BETA, RC)
    ...
    do i = 1, max_it - 1
       ...
       call j_matrices(para, i, Aj,Bj,Cj,Fj,Gj)
    ...
    do i = 1,neps
       MA_VECTOR(:,i) = MA_VECTOR2((i-1)*(max_it+1)*neq+1:i*(max_it+1)*neq)
    end do
